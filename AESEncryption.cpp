...
/*SOURCES: https://en.wikipedia.org/wiki/Rijndael_S-box
...
//rijndael's mix columns for multiplying by 2
unsigned char mul2[] = {
...
//rijndael's mix columns for multiplying by 3
unsigned char mul3[] = {
...
void KeyExpansion(unsigned char* input, unsigned char* expanded){
  for (int i = 0; i < 16; i++)
  {
    expanded[i] = input[i];
  }
  ...
  while(bytesGenerated < 176)
  {
    ...
    {
      expanded[bytesGenerated] = expanded[bytesGenerated - 16] ^ temp[a];
      bytesGenerated++;
    }
...
void AES_Encrypt(unsigned char* message, unsigned char* key){
  ...
  for(int i = 0; i < numRounds; i++)
  {
    ...
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(state, expanded + (16 * ( i+1 )));
  }
  ...
  //Encrytping over original message
...
int main(){
  ...
  unsigned char message[] = "This message will be encrypted and you will not see it.";
